...
# roc curves
def plot_roc(fig,ax1,ax2,y_true, y_score, y_weight,threshold,acceptance,frac_hh4b_pass, y_hh4b, estimatorName='train',lw=1,color="black",linestyle='solid',alpha_i=1,fancy_plot = False,do_zoom=False):
    ...
    if fancy_plot == False:
        ...
        ax1.scatter(tpr[idx_fixAcc], fpr[idx_fixAcc], label = "fixed signal acceptance threshold: %s hh4b pass: %.2f" % (round(threshold_fixAcc,2), round(frac_hh4b_pass_fixAcc, 2)) ,color=color, marker='s')
        ...
        ax1.scatter(tpr_noZeros[idx_fixAcc], div[idx_fixAcc], label = "fixed signal acceptance threshold: %s  pass: %.2f" % (round(threshold_fixAcc,2), round(frac_hh4b_pass_fixAcc, 2)) ,color=color,marker='s')
